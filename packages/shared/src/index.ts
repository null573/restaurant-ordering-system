...
export interface WSEvents {
  'order:new': { orderId: string; deskId: string; deskNumber: string; items: { dishName: string; quantity: number; remark: string | null }[] };
  'order:status': { orderId: string; status: OrderStatus };
  'desk:status': { deskId: string; status: DeskStatus };
  'payment:done': { orderId: string; payMethod: PayMethod };
  'item:status': { itemId: string; status: 'new' | 'cooking' | 'done' };
}
...
export interface Receipt {
  ...
  orderNo: string;
  deskNumber: string;
  items: { name: string; quantity: number; priceFen: number; subtotalFen: number }[];
  totalFen: number;
  payMethod: PayMethod | null;
  paidFen: number;
  changeFen: number;
